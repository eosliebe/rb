...
if (!$siteid)
{
		...
		$QVAL = "'$gid','$id','$name','$name','0','kf-home','".$layout."','".$startpage."','','','korean','1','html5','nic','0','0','0','1','',''";
	...
	getDbUpdate($table['s_site'],"layout='".$layout."',startpage='".$startpage."'",'uid='.$S['uid']);
...
{
	$qkey = 'gid,site,isson,parent,depth,id,menutype,mobile,hidden,reject,name,target,redirect,joint,layout,imghead,imgfoot,puthead,putfoot';
...
getLink($g['s'].'/?r='.$r.'&iframe=Y&m=admin&module=module&front=modal.package&package_type='.$package_type.'&package_step=3&siteid='.$id.'&site_name='.urlencode($name).'&package_name='.urlencode($package_name),'parent.','','');
