...
if ($cat && !$vtype)
{
	...
	$QVAL.= "layout='$layout',imghead='$imghead',imgfoot='$imgfoot',addattr='$addattr',addinfo='$addinfo'";
	getDbUpdate($table['s_menu'],$QVAL,'uid='.$cat);
	
	$_SEO = getDbData($table['s_seo'],'uid='.(int)$seouid,'uid');
	if($_SEO['uid']) getDbUpdate($table['s_seo'],"title='$title',keywords='$keywords',description='$description'",'uid='.$_SEO['uid']);
	else getDbInsert($table['s_seo'],'rel,parent,title,keywords,description',"'1','$cat','$title','$keywords','$description'");
	...
	for ($i = 0 ; $i < $slen; $i++)
	{
		...
		$QKEY = "gid,site,isson,parent,depth,id,menutype,mobile,hidden,reject,name,target,redirect,joint,perm_g,perm_l,layout,imghead,imgfoot,addattr,num,d_last,addinfo";
		$QVAL = "'$gid','$account','0','$parent','$xdepth','$xnarr[1]','$menutype','$mobile','$hidden','$reject','$xnarr[0]','$target','$redirect','$joint','$perm_g','$perm_l','$layout','','','$addattr','0','',''";

		getDbInsert($table['s_menu'],$QKEY,$QVAL);
		$lastmenu = getDbCnt($table['s_menu'],'max(uid)','');
		getDbInsert($table['s_seo'],'rel,parent,title,keywords,description',"'1','$lastmenu','$title','$keywords','$description'");
	...
	getLink($g['s'].'/?r='.$r.'&m=admin&module='.$m.'&front=menu&parent='.$parent.'&code='.$code.'&vtype='.$vtype,'parent.','','');
}
?>