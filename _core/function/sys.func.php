...
function isMobileConnect($agent)
{
	...
	foreach($_agents as $key => $val)
	{
		if(strpos($xagent,$key)) return $val;
	}
	return '';
}
//MOD_rewrite
function RW($rewrite)
{
	if ($GLOBALS['_HS']['rewrite'])
	{
		...
		$rewrite = str_replace('m=forum','b',$rewrite);
...
{
	global $g,$d;
	if ($kind == 'js') echo '<script src="'.$g['s'].'/_core/plugins/'.$plugin.'/'.($version?$version:$d['ov'][$plugin]).'/'.$path.'.js"></script>';
	else echo '<link href="'.$g['s'].'/_core/plugins/'.$plugin.'/'.($version?$version:$d['ov'][$plugin]).'/'.$path.'.css" rel="stylesheet">';
}
//썸네일(@ 2.0.0)
...
function getTreeMenu($conf,$cat,$code,$depth,$parent,$tmpcode)
{
	...
	while($C=db_fetch_array($CD))
	{
		...
		if ($C['isson'])
		{
			...
			if($C['mobile']) $tree.= '<i class="glyphicon glyphicon-phone" title="모바일"></i>';
			if($C['hidden']) $tree.= '<i class="glyphicon glyphicon-eye-close" title="숨김"></i>';
			if($C['reject']) $tree.= '<i class="glyphicon glyphicon-ban-circle" title="차단"></i>';
